...
export default function Righistory() {
  ...
  useEffect(() => {
    ...
    const handler = setTimeout( async () => {
        try {
          const response = await axios.get(`${process.env.NEXT_PUBLIC_URL}/payout/getpayouthistorysuperadmin?type=chronocoinwallet&searchUsername=${search}&page=${currentpage}&limit=10`,{
  ...
  return (
    <div className=' relative w-full flex flex-col items-center gap-8 max-w-[1440px] min-h-[500px] h-auto mt-12 bg-zinc-800 p-6'>
        <div className=' flex md:flex-row flex-col items-center justify-between absolute top-0 w-[98%] gap-2 h-auto md:h-[55px] bg-yellow-500 p-2 rounded-sm -translate-y-4'>
            <p className=' text-sm font-semibold text-black'>Chrono Package Payout History</p>
